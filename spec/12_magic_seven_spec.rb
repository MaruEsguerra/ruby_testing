...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 8 and the random number is 5' do
  ...
  describe '#play' do
    context 'when the random number is 21' do
      subject(:game_twenty_one) { described_class.new(21) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_twenty_one.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 101' do
      # remove the 'x' before running this test
      subject(:game_hundred_one) { described_class.new(101) }
      it 'will return 7' do
        result = game_hundred_one.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 7000' do
      # remove the 'x' before running this test
      subject(:game_seven_thousand) { described_class.new(7000) }
      it 'will return 7' do
        result = game_seven_thousand.play
        expect(result).to eq(7)
      end
    end
  end
