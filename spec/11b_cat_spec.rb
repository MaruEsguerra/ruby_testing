...
describe Cat do
  ...
  subject(:valkyrie) { described_class.new('Valkyrie', 'Persian', 'Black') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when using shared example for talk method' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(valkyrie).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(valkyrie).to be_hiding
  end
end
